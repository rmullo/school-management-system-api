...
exports.getAdmProfileCtrl = asyncHandler(async (req, res) => {
    const admin = await Admin.findById(req.user._id).select('name email role academicYears').populate('academicYears academicTerms classLevels programs subjects students teachers');
    if (!admin) {
